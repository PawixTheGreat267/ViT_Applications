...
from .utils import DropPath, Mlp, trunc_normal_
...
class PatchEmbedSwin(nn.Module):
    r""" Image to Patch Embedding for Swin Transformer
    """
...
def swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    """Swin-T @ 224x224"""
    model = SwinTransformer(patch_size=4, window_size=7, embed_dim=96, depths=[2, 2, 6, 2],
...
def swin_base_patch4_window7_224(pretrained=False, **kwargs):
    """Swin-B @ 224x224"""
    model = SwinTransformer(patch_size=4, window_size=7, embed_dim=128, depths=[2, 2, 18, 2],
